...
# Final : Added comments for later review
...
def main():
    """Main function of the application. Bagels a mastermind type game."""
...
def get_secret_number(length):
    """A function to generate a random string of digits of a given length."""
    base = [str(x) for x in range(0,10)]
    return ''.join(random.sample(base, length))

def check_answer(guess, goal):
    """A function to valide and respond to the players' guess."""
    result = []
